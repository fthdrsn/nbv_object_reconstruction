...
class TestCandidateViews(Node):
    ...
    def error_callback(self, msg):
        ...
        self.task_err=msg.data
        self.get_logger().debug(f'Received task error update: {self.task_err:.4f} m')
    ...
    def run_nbv_iteration(self):
        ...
        for best_view_idx in range(len(self.all_views)):
            ...
            self.publish_target_pose(best_view)     
            rclpy.spin_once(self, timeout_sec=0.01)
            while self.task_err>err_threshold and rclpy.ok():            
